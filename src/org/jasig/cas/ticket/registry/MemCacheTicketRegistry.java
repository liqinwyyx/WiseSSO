...
/**
 * Key-value ticket registry implementation that stores tickets in memcached
 * keyed on the ticket ID.
 * 
...
public final class MemCacheTicketRegistry extends
	 ...
	 * 
	 * @param hostnames
	 *            Array of memcached hosts where each element is of the form
	 *            host:port.
	 ...
	 */
	public MemCacheTicketRegistry(final String[] hostnames,
			final int ticketGrantingTicketTimeOut,
			final int serviceTicketTimeOut) {
		try {
			this.client = new MemcachedClient(AddrUtil.getAddresses(Arrays
					.asList(hostnames)));
		} catch (final IOException e) {
			throw new IllegalArgumentException(
					"Invalid memcached host specification.", e);
		}
	 ...
	 * timeout parameters in order to create a unique method signature.
	 * 
	 * @param ticketGrantingTicketTimeOut
	 *            TGT timeout in milliseconds.
	 * @param serviceTicketTimeOut
	 *            ST timeout in milliseconds.
	 * @param hostnames
	 *            Array of memcached hosts where each element is of the form
	 *            host:port.
	 ...
	 */
	@Deprecated
	public MemCacheTicketRegistry(final long ticketGrantingTicketTimeOut,
			final long serviceTicketTimeOut, final String[] hostnames) {
		this(hostnames, (int) (ticketGrantingTicketTimeOut / 1000),
				(int) (serviceTicketTimeOut / 1000));
	}

	/**
	 * Creates a new instance using the given memcached client instance, which
	 * is presumably configured via
	 ...
	 *            ST timeout in seconds.
	 */
	public MemCacheTicketRegistry(final MemcachedClient client,
			final int ticketGrantingTicketTimeOut,
			final int serviceTicketTimeOut) {
		this.tgtTimeout = ticketGrantingTicketTimeOut;
		this.stTimeout = serviceTicketTimeOut;
	...
	protected void updateTicket(final Ticket ticket) {
		...
		try {
			if (!this.client
					.replace(ticket.getId(), getTimeout(ticket), ticket).get()) {
	...
	/**
	 * As of version 3.5, this operation has no effect since async writes can
	 * cause registry consistency issues.
	 */
	@Deprecated
